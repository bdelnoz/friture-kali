...
do_exec() {
    ...
    run_cmd "pip install runtime deps" pip install --upgrade numpy scipy pyqtgraph sounddevice appdirs
    run_cmd "pip install friture without build isolation" pip install --no-build-isolation friture
